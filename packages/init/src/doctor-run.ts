...
function extractPareServers(client: ClientEntry, projectDir: string): Map<string, ServerConfig> {
  ...
  try {
    ...
    console.warn(`Warning: Could not parse ${resolveConfigPath(client.configPath, projectDir)}`);
  }

  return servers;
}

/** Run the doctor health check. Accepts pre-parsed argv (without the "doctor" positional). */
